#!/usr/bin/env python3
"""
Persistent Neo4j Deployment with EFS using AWS MCP Cloud Control
"""
...
# Import the MCP client from the base integration
from mcp_integration import MCPCloudControlClient

@dataclass
class PersistentDeploymentResult:
    """Result of a persistent deployment operation"""
    resource_type: str
    identifier: str
    status: str
...
class PersistentNeo4jDeployer:
    """
    Neo4j deployer with EFS persistence and direct port mapping
    """
    ...
    async def initialize_session(self) -> Tuple[str, str]:
        ...
        logger.info("Initializing AWS session for persistent Neo4j deployment...")
    ...
    async def deploy_persistent_infrastructure(self) -> Dict:
        """Deploy complete persistent Neo4j infrastructure using MCP"""
        logger.info("🚀 Starting persistent Neo4j infrastructure deployment")
        ...
        try:
            ...
            # 4. Deploy task definition with EFS volume
            logger.info("📋 Deploying ECS task definition with EFS persistence...")
            container_config = self.config["container"]
            efs_config = self.config["efs"]
            
            # Build container definition with mount points
            container_def = {
                "name": "database",
                ...
                "mountPoints": [
                    ...
                    } for mp in container_config.get("mount_points", [])
                ],
                "volumesFrom": [],
                "systemControls": []
            }
            
            # Build volumes configuration with EFS
            volumes = []
            for volume in self.config["ecs"].get("volumes", []):
                if "efs_volume_configuration" in volume:
                    volumes.append({
                        "name": volume["name"],
                        "efsVolumeConfiguration": {
                            "fileSystemId": volume["efs_volume_configuration"]["file_system_id"],
            ...
            task_def_result = await self.deploy_resource_with_mcp(
                ...
                credentials_token=creds_token,
                context="ECS Task Definition with EFS"
            )
            ...
            # 6. Deploy target groups (both HTTP and TCP for Bolt)
            ...
            for tg_config in lb_config["target_groups"]:
                ...
                if tg_config["protocol"] == "HTTP":
                    tg_properties["HealthCheckPath"] = tg_config["health_check"]["path"]
                    tg_properties["Matcher"] = {"HttpCode": tg_config["health_check"]["matcher"]}
                
                tg_result = await self.deploy_resource_with_mcp(
                    resource_type="AWS::ElasticLoadBalancingV2::TargetGroup",
                    properties=tg_properties,
                    credentials_token=creds_token,
            ...
            # 7. Deploy listeners (direct port mapping)
            logger.info("👂 Deploying ALB listeners with direct port mapping...")
            ...
            # 8. Deploy ECS service with both target groups
            logger.info("🚀 Deploying ECS service with load balancer integration...")
            service_config = self.config["ecs"]["service"]
            
            # Configure load balancers for both ports
            load_balancers = [
                {
                    "TargetGroupArn": self.resource_mappings["bigfootgolf-neo4j-http-tg"],
                    "ContainerName": "database",
                    "ContainerPort": 7474
                },
                {
                    "TargetGroupArn": self.resource_mappings["bigfootgolf-neo4j-bolt-tg"],
                    "ContainerName": "database",
                    "ContainerPort": 7687
                }
            ]
            ...
            logger.info("✅ Persistent Neo4j infrastructure deployment completed successfully!")
            
            # Generate ALB DNS name
            ...
            return {
                ...
                "endpoints": {
                    "neo4j_browser": f"http://{alb_dns}:7474/",
                    "neo4j_bolt": f"bolt://{alb_dns}:7687",
                    "neo4j_http": f"http://{alb_dns}:7474",
                    "note": "Direct port access (7474/7687) with EFS persistent storage"
                },
                "persistence": {
                    "efs_file_system": self.config["efs"]["file_system_id"],
                    "mount_path": self.config["efs"]["mount_path"],
                    "encryption": self.config["efs"]["transit_encryption"]
                }
            }
...
def main():
    """Main function for persistent Neo4j deployment"""
    import sys
    
    if len(sys.argv) != 2:
        print("Usage: python neo4j_persistent_deployer.py <config_path>")
        sys.exit(1)
    
    config_path = sys.argv[1]
    
    async def run_deployment():
        deployer = PersistentNeo4jDeployer(config_path)
        results = await deployer.deploy_persistent_infrastructure()
        
        print("\n" + "="*80)
        print("🎉 PERSISTENT NEO4J DEPLOYMENT COMPLETED SUCCESSFULLY!")
        print("="*80)
        print(json.dumps(results, indent=2))
        print("\n🌐 Access your persistent Neo4j database at:")
        for name, endpoint in results["endpoints"].items():
            if name != "note":
                print(f"   {name}: {endpoint}")
        print(f"\n💾 Data is persisted on EFS: {results['persistence']['efs_file_system']}")
        print(f"📁 Mount path: {results['persistence']['mount_path']}")
