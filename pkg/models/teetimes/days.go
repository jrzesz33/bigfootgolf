...
func (r *ReservedDay) GetBySlot(slot int64) *Reservation {
	for _, res := range r.Times {
		if res.Slot == slot {
			return &res
		}
	}
...
func (r *ReservedDay) GetByTime(hour, minute int) *Reservation {
	for _, res := range r.Times {
		if res.TeeTime.Hour() == hour && res.TeeTime.Minute() == minute {
			return &res
		}
	}
	return nil
