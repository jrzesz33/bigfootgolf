...
func (te *ToolExecutor) bookTeeTime(input map[string]interface{}) (string, error) {
	...
	var be teetimes.BookingEngine
	resDay, _ := be.GetDayTeeTimes(date)
	for _, resDay := range resDay[0].Times {
		if resDay.TeeTime.Hour() == hour && resDay.TeeTime.Minute() == minute {
			resDay.BookingUser = &account.User{ID: te.UserID}
			err := teetimes.BookTeeTime(resDay)
			if err != nil {
...
func (te *ToolExecutor) getWeatherForecast(input map[string]interface{}) (string, error) {
	...
	var result strings.Builder
	result.WriteString(fmt.Sprintf("Golf course weather forecast:\n"))
